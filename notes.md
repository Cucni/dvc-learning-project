...
## Accessing and sharing data, models and experiments

DVC includes functionalities for easy accessing and sharing data, models and experiments. Since the DVC version control is not actually specific for data but works for any file, the base features work in the same way for all types of files, be they datasets or models (for example serialized models).

To get a file from a DVC repo we can use `dvc get <path>`. This fetches the file once to our local repo. Note that this can be considered a high-level interface for downloading files with all kinds of protocols (no need to distinguish between using `wget`, `curl` or other tools).

However, it is often the case that we want to keep track of where we downloaded a file from, and have a way to check if it has changed and re-download it in this case. This is achieved with the command `dvc import <path>`. This will fetch the file from a DVC repository and store it in the local repo, but will also take of setting up tracking, so it will create a `<path>.dvc` file and add the necessary entries to `.gitignore` if required. By inspecting the `.dvc` file we can notice that in addition to tracking the file as usual (with path and hash), DVC is also tracking the URL of the DVC repo we downloaded it from, and a versioning hash that is used to check if the file at the remote repo has changed or not. This allows us to check if the file has been updated and re-download it if so, with the command `dvc update <path>.dvc`.

Note that `dvc import` and `dvc import-url` achieve similar goals: to obtain a file from a remote location, add it to our local repository and start tracking it. However, `dvc import` is used to specifically target DVC repositories, while `dvc import-url` is used to obtain files from any URL.

DVC also offers a python API that can be used instead of the CLI commands to obtain files from a DVC repo. The main advantages of using the python API are that it is possible to do all the operations in the same scripts where the pipeline is implemented (thus avoiding the need for running CLI commands) and the existence of context managers that handle temporary file downloads. These behave identically to the standard context managers for handling files: it is possible to use `with dvc.api.open(<path>, <repo_url>) as f` to obtain a file from a DVC repo. The file will be downloaded automatically when the script is executed, but instead of being stored in the local workspace it will be removed after exiting the context manager. For this reason, it is particularly useful to obtain files that does not make sense to persist in the local context (for example, models for the sole purpose of making predictions).

All of the commands and the API provide functionalities for targeting specific git revisions and fetch the desired files.

The above works for obtaining files from another DVC repo.

The above works for obtaining files from another DVC repo. However, we can also ask ourselves how to share (or in general store in a centralized remote for later retrieval) our data, models and experiments. For data that we added to DVC tracking with `dvc add <path>`, this is part of the basic workflow: the commands `dvc push` and `dvc pull` will push/pull our local copies and cache to the remote. It turns out that this is the case for metrics and plots too. Since they are tracked in the cache, whenever the cache is sent/retrieved to/from the remote, they are also shared and can be exchanged in this way.

This is however not sufficient for experiments. Remember that experiments package together all the trackable pieces of a pipeline, and are implemented internally as git refs. These git refs exist only on the local copy of our git repository. Therefore, it does not suffice to synchronize the cache with the remote (which would synchronize the DVC cache), but we also need to synchronize the git remote in order to push these "special refs". For this reason, experiments have their own command instead: `dvc exp push <remote>`. This takes care of pushing the cache to the DVC remote, and the special git refs to the git remote, thus managing two remotes at once (git + DVC). Indeed, note that the output of `dvc exp push` mentions pushing experiments to the _git_ remote, not to the _DVC_ remote--even though it is also pushing the DVC cache to the DVC remote.

Experiments stored in the cache or in the remote are still not persistent though, as more than one experiment can be attached to a given git commit. This happens for example when we run hyperparameter search: except for one choice of parameters, the others will end up being experiments attached to the current commit but that use different choices of parameters than that we obtain from checking out the commit. In this sense, only the run with the status identical to the committed one is persistent. To properly make an experiment persistent, one can commit the workspace state to git; but this would "override" the previous state in the current branch, so a second possibility is to create a new branch and create a commit there instead (so that the committed state and the new experiment state are on separate branches). This can be achieved with `dvc exp branch`. For example, this covers use cases in which for we want to keep the original experiment as main/stable/prod while keep developing the newly found one.
...
_The content of this section should be propagated above, with only a final remark remaining here. The notes currently explain this part incorrectly, as it can be guessed by the intricate explanations._

It is not true that outpus are tracked "only by hash in dvc.lock". Outputs files, metrics and plots are properly tracked with DVC like all other files that are added via dvc add. The difference is that there is no .dvc file, but they are properly tracked and that's why you can get them back. Indeed, there is no need for a .dvc file because the hash is stored in the dvc.lock file!

This is also confirmed in the official documentation at https://dvc.org/doc/command-reference/stage/add#dependencies-and-outputs, which explains the following options:
...
This confirms that outputs are tracked by DVC. The same is true for metrics and plots. This also means that they will be pushed to a DVC remote whenever you run `dvc push`, and that you can use `dvc pull` to obtain them!