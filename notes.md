...
Experiments stored in the cache or in the remote are still not persistent though, as more than one experiment can be attached to a given git commit. This happens for example when we run hyperparameter search: except for one choice of parameters, the others will end up being experiments attached to the current commit but that use different choices of parameters than that we obtain from checking out the commit. In this sense, only the run with the status identical to the committed one is persistent. To properly make an experiment persistent, one can commit the workspace state to git; but this would "override" the previous state in the current branch, so a second possibility is to create a new branch and create a commit there instead (so that the committed state and the new experiment state are on separate branches). This can be achieved with `dvc exp branch`.
